...
function addPercent() {
    ...
    if ($html == html) {
        ...
        if ($css == css) {
            ...
            $('.css3').find('.percent').text(i);
        }
        if ($js == js) {
            $('.js').find('.percent').text(js);
        } else {
            $('.js').find('.percent').text(i);
        }
        if ($vue == vue) {
...
function section5() {
    ...
    move('.my-course-mail .content-box').set('transform', 'rotateY(0deg)').set('opacity', 1).duration('1.9s').end();
    move('.my-course-erp .content-box').set('transform', 'rotateY(0deg)').set('opacity', 1).duration('2.1s').end();
}
//section6动画
function section6() {
...
$(document).ready(function () {
    ...
    console.log("要不打个电话试试:%c 180-6981-9809", "color:red");
});//readyfunction结束
